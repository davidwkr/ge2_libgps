#!/bin/sh
# SL8541E GNSS Standalone Fix - MOVISTAR (No Control Plane + WiFi Optimized)
# This version uses INTERFACE="any" to allow SUPL over WiFi in weak LTE areas.

echo "--- Starting standalone GNSS Fix (Movistar No-CP + WiFi) ---"
...
# Log On-Device
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Applied MOVISTAR No-CP + WiFi Fix" >> /data/gnss/fix_history.log

echo "--- WiFi-Optimized No-CP Fix Applied. Please REBOOT. ---"
